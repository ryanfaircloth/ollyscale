...
down_revision: str | Sequence[str] | None = "c846417c2785"
...
def upgrade() -> None:
    ...
    - Fact tables and their associated resources (otel_resources_dim)
    - Fact tables and their instrumentation scopes (otel_scopes_dim)
    - Metrics data points and metric metadata (otel_metrics_dim)
    """
    ...
    op.execute("""
        ...
        REFERENCES otel_resources_dim(resource_id)
    """)
    ...
    op.execute("""
        ...
        REFERENCES otel_scopes_dim(scope_id)
    """)
    ...
    op.execute("""
        ...
        REFERENCES otel_metrics_dim(metric_id)
    """)
...
def downgrade() -> None:
    ...
    # Spans
    op.execute("ALTER TABLE otel_spans_fact DROP CONSTRAINT IF EXISTS fk_spans_scope")
    op.execute("ALTER TABLE otel_spans_fact DROP CONSTRAINT IF EXISTS fk_spans_resource")

    # Logs
    op.execute("ALTER TABLE otel_logs_fact DROP CONSTRAINT IF EXISTS fk_logs_scope")
    op.execute("ALTER TABLE otel_logs_fact DROP CONSTRAINT IF EXISTS fk_logs_resource")

    # Metrics - Number
    op.execute("ALTER TABLE otel_metrics_data_points_number DROP CONSTRAINT IF EXISTS fk_metrics_number_scope")
    op.execute("ALTER TABLE otel_metrics_data_points_number DROP CONSTRAINT IF EXISTS fk_metrics_number_resource")
    op.execute("ALTER TABLE otel_metrics_data_points_number DROP CONSTRAINT IF EXISTS fk_metrics_number_metric")

    # Metrics - Histogram
    op.execute("ALTER TABLE otel_metrics_data_points_histogram DROP CONSTRAINT IF EXISTS fk_metrics_histogram_scope")
    op.execute("ALTER TABLE otel_metrics_data_points_histogram DROP CONSTRAINT IF EXISTS fk_metrics_histogram_resource")
    ...
    op.execute(
        "ALTER TABLE otel_metrics_data_points_exp_histogram DROP CONSTRAINT IF EXISTS fk_metrics_exp_histogram_metric"
    )

    # Metrics - Summary
    op.execute("ALTER TABLE otel_metrics_data_points_summary DROP CONSTRAINT IF EXISTS fk_metrics_summary_scope")
    op.execute("ALTER TABLE otel_metrics_data_points_summary DROP CONSTRAINT IF EXISTS fk_metrics_summary_resource")
    op.execute("ALTER TABLE otel_metrics_data_points_summary DROP CONSTRAINT IF EXISTS fk_metrics_summary_metric")
