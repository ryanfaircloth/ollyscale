...
def upgrade() -> None:
    ...
    op.execute("COMMENT ON COLUMN otel_log_attrs_string.log_id IS 'Foreign key to otel_logs_fact'")
    op.execute("COMMENT ON COLUMN otel_log_attrs_string.key_id IS 'Foreign key to attribute_keys for attribute name'")
    op.execute("COMMENT ON COLUMN otel_log_attrs_string.value IS 'String attribute value'")

    op.execute(
        "COMMENT ON TABLE otel_log_attrs_int IS 'Integer-typed log attributes stored in dedicated table for query performance'"
    )
    op.execute("COMMENT ON COLUMN otel_log_attrs_int.log_id IS 'Foreign key to otel_logs_fact'")
    op.execute("COMMENT ON COLUMN otel_log_attrs_int.key_id IS 'Foreign key to attribute_keys for attribute name'")
    op.execute(
    ...
    op.execute("COMMENT ON COLUMN otel_log_attrs_double.log_id IS 'Foreign key to otel_logs_fact'")
    op.execute("COMMENT ON COLUMN otel_log_attrs_double.key_id IS 'Foreign key to attribute_keys for attribute name'")
    op.execute("COMMENT ON COLUMN otel_log_attrs_double.value IS 'Double precision floating point attribute value'")

    op.execute(
        "COMMENT ON TABLE otel_log_attrs_bool IS 'Boolean-typed log attributes stored in dedicated table for query performance'"
    )
    op.execute("COMMENT ON COLUMN otel_log_attrs_bool.log_id IS 'Foreign key to otel_logs_fact'")
    op.execute("COMMENT ON COLUMN otel_log_attrs_bool.key_id IS 'Foreign key to attribute_keys for attribute name'")
    op.execute("COMMENT ON COLUMN otel_log_attrs_bool.value IS 'Boolean attribute value'")

    op.execute("COMMENT ON TABLE otel_log_attrs_bytes IS 'Binary/bytes log attributes stored in dedicated table'")
    op.execute("COMMENT ON COLUMN otel_log_attrs_bytes.log_id IS 'Foreign key to otel_logs_fact'")
    op.execute("COMMENT ON COLUMN otel_log_attrs_bytes.key_id IS 'Foreign key to attribute_keys for attribute name'")
    op.execute("COMMENT ON COLUMN otel_log_attrs_bytes.value IS 'Binary attribute value stored as BYTEA'")

    op.execute(
        "COMMENT ON TABLE otel_log_attrs_other IS 'JSONB catch-all for unpromoted, complex, or array-typed log attributes'"
    )
    op.execute("COMMENT ON COLUMN otel_log_attrs_other.log_id IS 'Primary key and foreign key to otel_logs_fact'")
    op.execute(
