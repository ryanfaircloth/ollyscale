...
def upgrade() -> None:
    ...
    Foreign keys added:
    - otel_logs_fact → otel_spans_fact (trace correlation)
    - otel_spans_fact → otel_spans_fact (parent-child relationships)
    - otel_span_links → otel_spans_fact (linked span backreference)
    """

    # First, add unique constraint on otel_spans_fact for FK target
    ...
    op.execute("""
        ...
        REFERENCES otel_spans_fact(trace_id, span_id_hex)
        NOT VALID
    """)
    ...
    # FK from otel_span_links to linked span (backreference)
    # Allows traversing span links in reverse
    op.execute("""
        ALTER TABLE otel_span_links
        ADD CONSTRAINT fk_otel_span_links_target
...
def downgrade() -> None:
    ...
    op.execute("ALTER TABLE otel_span_links DROP CONSTRAINT IF EXISTS fk_otel_span_links_target")
    op.execute("ALTER TABLE otel_spans_fact DROP CONSTRAINT IF EXISTS fk_spans_parent")
    op.execute("ALTER TABLE otel_logs_fact DROP CONSTRAINT IF EXISTS fk_logs_span_correlation")
    op.execute("ALTER TABLE otel_spans_fact DROP CONSTRAINT IF EXISTS uq_spans_trace_span_id")
