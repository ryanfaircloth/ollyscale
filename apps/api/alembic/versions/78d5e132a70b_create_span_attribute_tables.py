...
def upgrade() -> None:
    ...
    op.execute("COMMENT ON COLUMN otel_span_attrs_string.span_id IS 'Foreign key to otel_spans_fact'")
    op.execute("COMMENT ON COLUMN otel_span_attrs_string.key_id IS 'Foreign key to attribute_keys for attribute name'")
    op.execute("COMMENT ON COLUMN otel_span_attrs_string.value IS 'String attribute value'")

    op.execute(
        "COMMENT ON TABLE otel_span_attrs_int IS 'Integer-typed span attributes stored in dedicated table for query performance'"
    )
    op.execute("COMMENT ON COLUMN otel_span_attrs_int.span_id IS 'Foreign key to otel_spans_fact'")
    op.execute("COMMENT ON COLUMN otel_span_attrs_int.key_id IS 'Foreign key to attribute_keys for attribute name'")
    op.execute(
    ...
    op.execute("COMMENT ON COLUMN otel_span_attrs_double.span_id IS 'Foreign key to otel_spans_fact'")
    op.execute("COMMENT ON COLUMN otel_span_attrs_double.key_id IS 'Foreign key to attribute_keys for attribute name'")
    op.execute("COMMENT ON COLUMN otel_span_attrs_double.value IS 'Double precision floating point attribute value'")

    op.execute(
        "COMMENT ON TABLE otel_span_attrs_bool IS 'Boolean-typed span attributes stored in dedicated table for query performance'"
    )
    op.execute("COMMENT ON COLUMN otel_span_attrs_bool.span_id IS 'Foreign key to otel_spans_fact'")
    op.execute("COMMENT ON COLUMN otel_span_attrs_bool.key_id IS 'Foreign key to attribute_keys for attribute name'")
    op.execute("COMMENT ON COLUMN otel_span_attrs_bool.value IS 'Boolean attribute value'")

    op.execute("COMMENT ON TABLE otel_span_attrs_bytes IS 'Binary/bytes span attributes stored in dedicated table'")
    op.execute("COMMENT ON COLUMN otel_span_attrs_bytes.span_id IS 'Foreign key to otel_spans_fact'")
    op.execute("COMMENT ON COLUMN otel_span_attrs_bytes.key_id IS 'Foreign key to attribute_keys for attribute name'")
    op.execute("COMMENT ON COLUMN otel_span_attrs_bytes.value IS 'Binary attribute value stored as BYTEA'")

    op.execute(
        "COMMENT ON TABLE otel_span_attrs_other IS 'JSONB catch-all for unpromoted, complex, or array-typed span attributes'"
    )
    op.execute("COMMENT ON COLUMN otel_span_attrs_other.span_id IS 'Primary key and foreign key to otel_spans_fact'")
    op.execute(
    ...
    op.execute("COMMENT ON COLUMN otel_span_event_attrs_string.event_id IS 'Foreign key to otel_span_events'")
    op.execute(
        "COMMENT ON COLUMN otel_span_event_attrs_string.key_id IS 'Foreign key to attribute_keys for attribute name'"
    )
    op.execute("COMMENT ON COLUMN otel_span_event_attrs_string.value IS 'String attribute value'")
    ...
    op.execute("COMMENT ON COLUMN otel_span_event_attrs_int.event_id IS 'Foreign key to otel_span_events'")
    op.execute(
    ...
    op.execute("COMMENT ON COLUMN otel_span_event_attrs_double.event_id IS 'Foreign key to otel_span_events'")
    op.execute(
    ...
    op.execute("COMMENT ON COLUMN otel_span_event_attrs_bool.event_id IS 'Foreign key to otel_span_events'")
    op.execute(
        "COMMENT ON COLUMN otel_span_event_attrs_bool.key_id IS 'Foreign key to attribute_keys for attribute name'"
    )
    op.execute("COMMENT ON COLUMN otel_span_event_attrs_bool.value IS 'Boolean attribute value'")

    op.execute(
        "COMMENT ON TABLE otel_span_event_attrs_bytes IS 'Binary/bytes span event attributes stored in dedicated table'"
    )
    op.execute("COMMENT ON COLUMN otel_span_event_attrs_bytes.event_id IS 'Foreign key to otel_span_events'")
    op.execute(
        "COMMENT ON COLUMN otel_span_event_attrs_bytes.key_id IS 'Foreign key to attribute_keys for attribute name'"
    )
    op.execute("COMMENT ON COLUMN otel_span_event_attrs_bytes.value IS 'Binary attribute value stored as BYTEA'")
    ...
    op.execute("COMMENT ON COLUMN otel_span_link_attrs_string.link_id IS 'Foreign key to otel_span_links'")
    op.execute(
        "COMMENT ON COLUMN otel_span_link_attrs_string.key_id IS 'Foreign key to attribute_keys for attribute name'"
    )
    op.execute("COMMENT ON COLUMN otel_span_link_attrs_string.value IS 'String attribute value'")
    ...
    op.execute("COMMENT ON COLUMN otel_span_link_attrs_int.link_id IS 'Foreign key to otel_span_links'")
    op.execute(
    ...
    op.execute("COMMENT ON COLUMN otel_span_link_attrs_double.link_id IS 'Foreign key to otel_span_links'")
    op.execute(
    ...
    op.execute("COMMENT ON COLUMN otel_span_link_attrs_bool.link_id IS 'Foreign key to otel_span_links'")
    op.execute(
        "COMMENT ON COLUMN otel_span_link_attrs_bool.key_id IS 'Foreign key to attribute_keys for attribute name'"
    )
    op.execute("COMMENT ON COLUMN otel_span_link_attrs_bool.value IS 'Boolean attribute value'")

    op.execute(
        "COMMENT ON TABLE otel_span_link_attrs_bytes IS 'Binary/bytes span link attributes stored in dedicated table'"
    )
    op.execute("COMMENT ON COLUMN otel_span_link_attrs_bytes.link_id IS 'Foreign key to otel_span_links'")
    op.execute(
        "COMMENT ON COLUMN otel_span_link_attrs_bytes.key_id IS 'Foreign key to attribute_keys for attribute name'"
    )
    op.execute("COMMENT ON COLUMN otel_span_link_attrs_bytes.value IS 'Binary attribute value stored as BYTEA'")
