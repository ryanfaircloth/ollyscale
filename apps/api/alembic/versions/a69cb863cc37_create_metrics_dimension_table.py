...
def upgrade() -> None:
    ...
    op.execute(
        "COMMENT ON TABLE otel_metrics_dim IS 'Dimension table for OpenTelemetry metrics metadata with two-hash deduplication strategy supporting description variants'"
    )
    op.execute(
    ...
    op.execute("COMMENT ON COLUMN otel_metrics_dim.name IS 'OTLP metric name as defined by instrumentation'")
    op.execute(
        "COMMENT ON COLUMN otel_metrics_dim.metric_type_id IS 'Foreign key to metric_types (Gauge, Sum, Histogram, etc.)'"
    )
    op.execute("COMMENT ON COLUMN otel_metrics_dim.unit IS 'OTLP metric unit (e.g., ms, bytes, 1)'")
    op.execute(
    ...
    op.execute("COMMENT ON COLUMN otel_metrics_dim.description IS 'Metric description, can vary across variants'")
    op.execute("COMMENT ON COLUMN otel_metrics_dim.schema_url IS 'OTLP schema URL for semantic convention versioning'")
    op.execute(
