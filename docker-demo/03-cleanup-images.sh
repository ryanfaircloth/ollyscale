...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Demo Apps - Local Docker Images Cleanup${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
# Find all demo-related images
echo -e "${BLUE}Searching for demo Docker images...${NC}"
echo ""

IMAGES=$(docker images --format "{{.Repository}}:{{.Tag}}" | grep -E "demo-frontend|demo-backend" || true)

if [ -z "$IMAGES" ]; then
    echo -e "${YELLOW}No demo-related images found${NC}"
    echo ""
...
echo ""
docker images | grep -E "REPOSITORY|demo-frontend|demo-backend"
echo ""

# Calculate total size
TOTAL_SIZE=$(docker images --format "{{.Repository}}:{{.Tag}} {{.Size}}" | grep -E "demo-frontend|demo-backend" | awk '{print $2}' | sed 's/MB//;s/GB/*1024/;s/KB\/1024/' | bc 2>/dev/null | awk '{sum+=$1} END {printf "%.1f", sum}' || echo "unknown")
...
REMAINING=$(docker images --format "{{.Repository}}:{{.Tag}}" | grep -E "demo-frontend|demo-backend" || true)
if [ -n "$REMAINING" ]; then
